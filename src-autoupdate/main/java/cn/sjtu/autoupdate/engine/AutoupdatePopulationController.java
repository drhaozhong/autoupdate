...
import fr.inria.astor.core.solutionsearch.population.TestCaseBasedFitnessPopulationController.FitnessComparator;

public class AutoupdatePopulationController implements PopulationController {
	...
	public List<ProgramVariant> selectProgramVariantsForNextGeneration(List<ProgramVariant> parentVariants,
		...
		List<ProgramVariant> dups = findDuplicatedVariants(parentVariants, childVariants);
		
		childVariants.removeAll(dups);
...
//		if(!newPopulation.contains(original)) {
...
//		}
	...
	private List<ProgramVariant> findDuplicatedVariants(List<ProgramVariant> parentVariants,
			List<ProgramVariant> childVariants) {
		ArrayList<ProgramVariant> dups = new ArrayList<ProgramVariant>();
		for(ProgramVariant pv:childVariants) {
			if(!isNewVariant(parentVariants, pv)) {
				dups.add(pv);
			}
		}
		return dups;
	...
	private boolean isNewVariant(List<ProgramVariant> temporalInstances, ProgramVariant newCopyVariant) {
		boolean bNew = true;
		for(ProgramVariant programVariant:temporalInstances) {
			if(isSame(programVariant, newCopyVariant)) {
				bNew = false;
				break;
			}
		}
	...
	private boolean isSame(ProgramVariant pv1, ProgramVariant pv2) {
		ArrayList<String> msgs1 = getCompilatinoErrors(pv1);
		ArrayList<String> msgs2 = getCompilatinoErrors(pv2);
		if(msgs1.size()!=msgs2.size()) {
			return false;
		}
		boolean bSame = true;
		for(String msg1:msgs1) {
			if(!msgs2.contains(msg1)) {
				bSame = false;
				break;
			}
		}
	...
	private ArrayList<String> getCompilatinoErrors(ProgramVariant pv) {
		ArrayList<String> msgs = new ArrayList<String>();
		for(ModificationPoint point:pv.getModificationPoints()) {
