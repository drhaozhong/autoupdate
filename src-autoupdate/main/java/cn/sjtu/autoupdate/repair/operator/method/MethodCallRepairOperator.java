...
public class MethodCallRepairOperator extends MethodRepairOperator{


	public MethodCallRepairOperator() {
	...
	private List<OperatorInstance> createOperatorInstances(ModificationPoint modificationPoint,
		...
		for(Object o:invoke.getArguments()) {
			...
			if(o instanceof CtFieldReadImpl) {
				...
				CtInvocationImpl parainvoke = (CtInvocationImpl)o;
				type = parainvoke.getType();
			}
			if(type!=null) {
	...
	private OperatorInstance createOperatorInstances(ModificationPoint modificationPoint, Object o,
		...
		if(o instanceof CtFieldReadImpl) {
			...
			instance = createOperatorInstances(modificationPoint, (CtInvocationImpl)o, executable);
		}
		return instance;
	}
	
	private OperatorInstance createOperatorInstances(ModificationPoint modificationPoint, CtInvocationImpl oldInvoc,
			CtExecutableReference executable) {
		CtInvocationImpl subInv = new CtInvocationImpl<>();
		subInv.setTarget(oldInvoc.getTarget());
		subInv.setExecutable(oldInvoc.getExecutable());
		CtInvocationImpl newInvoc = new CtInvocationImpl<>();
		newInvoc.setExecutable(executable);
		newInvoc.setTarget(subInv);
