...
import cn.sjtu.autoupdate.engine.RepairResultPair;
...
public abstract class AbstractMain {
	...
	static {
		...
		options.addOption(ExtensionPoints.TARGET_CODE_PROCESSOR.identifier, true,
				"(Optional) Indicates the class name of the process that selects target elements. They must extend from "
						...
						+ " The classes must be included in the classpath.");
		options.addOption("codemapping", true, "The mapping of API elements.");
		options.addOption("olddoc", true, "The document of the old library.");
		options.addOption("newdoc", true, "The document of the new library.");
		options.addOption("j2se", true, "The document of J2SE.");
		options.addOption("projectIdentifier", true, "The name of the project.");
	...
	public abstract RepairResultPair run(String location, String projectName, String dependencies, String packageToInstrument,
	...
	public boolean processArguments(String[] args) throws Exception {
		...
		if(cmd.hasOption("projectIdentifier")) {
			ConfigurationProperties.properties.setProperty("projectIdentifier",
					cmd.getOptionValue("projectIdentifier"));
		}		
		
		if(cmd.hasOption("codemapping")) {
			ConfigurationProperties.properties.setProperty("codemapping",
					cmd.getOptionValue("codemapping"));
		}
		
		if(cmd.hasOption("olddoc")) {
			ConfigurationProperties.properties.setProperty("olddoc",
					cmd.getOptionValue("olddoc"));
		}
		
		if(cmd.hasOption("newdoc")) {
			ConfigurationProperties.properties.setProperty("newdoc",
					cmd.getOptionValue("newdoc"));
		}
		
		if(cmd.hasOption("j2se")) {
			ConfigurationProperties.properties.setProperty("j2se",
					cmd.getOptionValue("j2se"));
		}
	...
	protected ProjectRepairFacade getProjectConfiguration(String location, String projectIdentifier, String method,
		...
		String originalProjectRoots = location + File.separator;
		...
		properties.setOriginalProjectRootDirs(originalProjectRoots);//zhh

	
		determineSourceFolders(properties, srcWithMain, originalProjectRoots);
		...
		if (!ConfigurationProperties.getPropertyBool("autocompile")) {
			String[] roots = originalProjectRoots.split(";");
			List<String> originalBin = new ArrayList<String>();
			List<String> originalBinTest = new ArrayList<String>();
			for(String originalProjectRoot:roots) {
				originalBin.addAll(determineBinFolder(originalProjectRoot,
						ConfigurationProperties.getProperty("binjavafolder")));
				originalBinTest.addAll(determineBinFolder(originalProjectRoot,
	...
	private List<String> determineSourceFolders(ProjectConfiguration properties, boolean srcWithMain,
			String originalProjectRoots) throws IOException {
...
//			if (sourceFolders.isEmpty()) {
	...
	//			String[] possibleSrcFolders = new String[] { (originalProjectRoot + File.separator + "src/main/java"),//zhh
	//					(originalProjectRoot + File.separator + "src/java"),
	//					(originalProjectRoot + File.separator + "src"), };
	
			String[] possibleSrcFolders = new String[] { (File.separator + "src/main/java"),
					(File.separator + "src/java"),
					(File.separator + "src"),
					(File.separator + ""),};
...
//			}
	...
	//			String[] possibleTestSrcFolders = new String[] { (originalProjectRoot + File.separator + "src/test/java"),//zhh
	//					(originalProjectRoot + File.separator + "src/test"),
	//					(originalProjectRoot + File.separator + "test"), };
				String[] possibleTestSrcFolders = new String[] { ( File.separator + "src/test/java"),
						( File.separator + "src/test"),
						( File.separator + "test"), };
...
//			if (Files.exists(fSrc.toPath())) { zhh
			if (fSrc.exists()) {
