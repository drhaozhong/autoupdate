...
import cn.sjtu.autoupdate.engine.AutoupdateEngine;
import cn.sjtu.autoupdate.engine.RepairResult;
...
public class AstorMain extends AbstractMain {
	...
	public AstorCoreEngine createEngine(ExecutionMode mode) throws Exception {
		...
		} else if(ExecutionMode.AutoUpdate.equals(mode)){
			astorCore = new AutoupdateEngine(mutSupporter, projectFacade);
		}else {
...
//		astorCore.setVariantFactory(new ProgramVariantFactory(astorCore.getTargetElementProcessors()));
		...
		if (ConfigurationProperties.getPropertyBool("skipfaultlocalization")) {
			...
			astorCore.initPopulation(new ArrayList<SuspiciousTestedCode>());
		} else {
			List suspicious = astorCore.calculateSuspicious();
	...
	@Override
	public RepairResultPair run(String location, String projectName, String dependencies, String packageToInstrument, double thfl,
			String failing) throws Exception {
		RepairResultPair pair = new RepairResultPair();
		...
		pair.beforeRepair = new RepairResult(astorCore.getInitSuspList());
		
		if(astorCore.getInitSuspList().size()>0) {
			pair.afterRepair = astorCore.startEvolution();
			astorCore.atEnd();
	...
	public RepairResultPair execute(String[] args) throws Exception {
		...
		RepairResultPair pair=null;
		try {
			pair = run(location, projectName, dependencies, packageToInstrument, thfl, failing);
		}catch(Exception e) {
			e.printStackTrace();
		}
		if(pair==null) {
			pair = new RepairResultPair();
		}
		if(pair.beforeRepair==null) {
			pair.beforeRepair = new RepairResult();
			pair.beforeRepair.setResult(new ArrayList<SuspiciousCompiledCode>(), -2);
		}
		if(pair.afterRepair==null) {
			pair.afterRepair = pair.beforeRepair;
		}
		System.out.println("Result:"+pair.beforeRepair.getFitness()+"->"+pair.afterRepair.getFitness()+":"+pair.afterRepair.id);
		return pair;
