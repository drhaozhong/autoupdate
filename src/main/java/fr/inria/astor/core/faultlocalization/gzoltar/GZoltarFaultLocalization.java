...
public class GZoltarFaultLocalization implements FaultLocalizationStrategy {
	...
	private FaultLocalizationResult calculateSuspicious(String locationSrc, String locationBytecode,
		...
		List<SuspiciousTestedCode> suspiciousStatemens = flResult.getCandidates();
		...
		if (ConfigurationProperties.getPropertyBool("filterfaultlocalization")) {
			List<SuspiciousTestedCode> filtercandidates = new ArrayList<SuspiciousTestedCode>();

			for (SuspiciousTestedCode suspiciousCode : suspiciousStatemens) {
	...
	protected FaultLocalizationResult searchSuspicious(String locationBytecode, List<String> testsToExecute,
		...
		List<SuspiciousTestedCode> candidates = new ArrayList<SuspiciousTestedCode>();
		...
		for (int i = 0; i < max; i++) {
			...
			SuspiciousTestedCode suspcode = new SuspiciousTestedCode(compName, gzoltarStatement.getMethod().toString(),
	...
	public class ComparatorCandidates implements Comparator<SuspiciousTestedCode> {

		@Override
		public int compare(SuspiciousTestedCode o1, SuspiciousTestedCode o2) {
			if (o1 == null || o2 == null) {
				return 0;
			}
