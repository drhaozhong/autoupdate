...
public class DynamothCollectorFacade implements ExecutionContextCollector {
	...
	public String[] getCoverTest(SuspiciousModificationPoint mp0) {
		String[] tests = null;
		if(mp0.getSuspicious() instanceof SuspiciousTestedCode) {
			SuspiciousTestedCode stc = (SuspiciousTestedCode)mp0.getSuspicious();
			tests = new String[stc.getCoveredByTests().size()];
			int i = 0;
			int nrfailing = 0;
			int nrpassing = 0;
			for (TestCaseResult tr : stc.getCoveredByTests()) {
