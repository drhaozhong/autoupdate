...
//				printMessage(p);
	...
	private void printMessage(Process p) {
	...
	private void killProcess(Process p, int waitTime) {
		...
		String os = System.getProperty("os.name");  
		if(os.toLowerCase().startsWith("win")){  
			KillWindowProcess(p, waitTime);
		}else {
			KillLinuxProcess(p, waitTime);
		}
	...
	private void KillLinuxProcess(Process p, int waitTime) {
		Object pid = null;
		try {
	...
	private void KillWindowProcess(Process process, int waitTime) {
		try {
		...
		for (int i = 0; i < urls.length; i++) {
			...
			if(os.toLowerCase().startsWith("win")){
				...
				if(url.getAuthority()!=null) {
					file += url.getAuthority();
				}
				file += url.getFile();
				if(file.startsWith("/")) {
					file = file.substring(1);
				}
				s += "\""+file+"\""+File.pathSeparator;
