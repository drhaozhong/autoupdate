...
public class CocoAstorFaulLocalizationTest {
	...
	public void cocoTest() throws Exception {
		...
		for (ModificationPoint mp : pv1.getModificationPoints()) {
			if( ((SuspiciousModificationPoint) mp).getSuspicious() instanceof SuspiciousTestedCode) {
				SuspiciousTestedCode stc = (SuspiciousTestedCode) ((SuspiciousModificationPoint) mp).getSuspicious();
				System.out.println(
					"mp " + mp + " " + stc.getSuspiciousValue());
				Assert.assertTrue(stc.getSuspiciousValue() < 1);
			}
		}
